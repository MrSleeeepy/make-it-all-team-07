...
if ($_SESSION["loggedin"] != true) {
  ...
  if ($postID != -1){
    //    //set the limitedVisibility of the post to 0
    $stmt = $conn->prepare("UPDATE Posts set limitedVisibility = 0 where postID = ? AND
...
//no post clicked so set the limited visibility off the topic to 0
    $stmt2 = $conn->prepare("UPDATE Topics set limitedVisibility = 0 where topicID = ? AND
    ((select manager from Users where userID = ?) = 1 or (select knowledgeAdmin from Users where userID = ?) = 1);");
